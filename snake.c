...
int main()
{
    initscr();//初始化了屏幕, 使之开始进入curses图形化工作方式，开始使用curses的一些命令

    /*start_color();//开启颜色，红色为前景色，绿色为背景色
    init_pair(1, COLOR_RED, COLOR_GREEN);
    attron(COLOR_PAIR(1));
    box(stdscr, ACS_VLINE, ACS_HLINE); //画一个框，stdsrc代表输出到标准屏幕
    ...
    */
    ...
    getOrder();
    endwin();
    return 0;//停止使用curses的一些命令
}

void initGame()
{
    cbreak();                    //把终端的CBREAK模式打开
    noecho();                    //关闭回显,非回显模式下按键通常用来控制屏幕的操作而不是用来进行字符输入
    curs_set(0);                //把光标置为不可见
...
//创建一个双向链表,head节点的next指针指向身体，其中head节点和tail节点只负责指向snake的头和尾，不存储自身坐标
void creatLink()
{
